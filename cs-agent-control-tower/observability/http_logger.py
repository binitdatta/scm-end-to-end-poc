...
def logged_request(method: str, service: str, url: str, *,
                    timeout: int, json_body: dict[str, Any] | None = None,
    ...
    `headers` (e.g. the Keycloak service-account Bearer token) is merged
    into the request but never written to the audit log/JSONL — only the
    body is recorded, so a token never lands in audit.db or logs/*.jsonl."""
