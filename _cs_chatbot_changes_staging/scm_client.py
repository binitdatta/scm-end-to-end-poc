...
def _get(service: str, path: str) -> dict[str, Any]:
    ...
    r = logged_request("GET", service, url, timeout=TIMEOUT)
    return r.json()


def _post(service: str, path: str, body: dict[str, Any]) -> dict[str, Any]:
    url = f"{_BASES[service]}{path}"
    r = logged_request("POST", service, url, timeout=TIMEOUT, json_body=body)
    return r.json()


def _put(service: str, path: str, body: dict[str, Any] | None = None) -> dict[str, Any]:
    url = f"{_BASES[service]}{path}"
    r = logged_request("PUT", service, url, timeout=TIMEOUT, json_body=body or {})
    return r.json()
